...
if (isset($_SESSION["user_id"])) {
	if (Core::$user->kind == 3) {
		...
		if (isset($_GET["opt"])) {
			$products = SellData::getSellsByStockId($_GET["opt"]);
		} else {
...
if (count($products) > 0) {
	...
	?>
		...
		?>
			...
			$c = ProductData::getById($operations[0]->product_id);
			//var_dump($c);
			echo $c->name;
		}?>
...
?>
	...
	function cambio(){
		if($("#controles").val()!=""){
			window.location.href="./?view=sells&opt="+$("#controles").val();
		}else{
			window.location.href="./?view=sells";
		}
	}
</script>
